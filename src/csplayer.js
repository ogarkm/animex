...
var parent = document.querySelector("#" + csPlayer.csPlayers[videoTag]["videoTag"].getIframe().id).closest(".csPlayer");
    var recommendationOverlay = parent.querySelector(".csPlayer-recommendation-overlay");

    if (!recommendationOverlay) {
        recommendationOverlay = document.createElement('div');
        recommendationOverlay.className = 'csPlayer-recommendation-overlay';
        recommendationOverlay.style.display = 'none'; // Initially hidden
        recommendationOverlay.style.position = 'absolute';
        recommendationOverlay.style.top = '0';
        recommendationOverlay.style.left = '0';
        recommendationOverlay.style.width = '100%';
        recommendationOverlay.style.height = '100%'; // Adjust to cover video area
        recommendationOverlay.style.background = 'rgba(0,0,0,0.5)'; // Example style
        recommendationOverlay.style.cursor = 'pointer';
        recommendationOverlay.innerHTML = '<i class="ti ti-player-play-filled" style="font-size: 50px; color: white; position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%);"></i>';
        parent.querySelector(".csPlayer-container").appendChild(recommendationOverlay);

        recommendationOverlay.addEventListener('click', function() {
            csPlayer.csPlayers[videoTag]["videoTag"].playVideo();
        });
    }
